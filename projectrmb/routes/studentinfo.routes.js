...
// ---------------------- GET ข้อมูลทั่วไปของนักเรียนคนหนึ่ง ----------------------
router.get('/:user_id', async (req, res) => {
  try {
    const [rows] = await pool.query(
      `SELECT student_user_id, form_data, avatar_url, created_at, updated_at
       FROM student_general_info
       WHERE student_user_id = ?`,
      [req.params.user_id]
    );

    if (!rows.length) {
      return res.status(404).json({ message: 'ยังไม่เคยกรอกแบบฟอร์ม' });
    }
...
router.put('/:user_id', async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO student_general_info (student_user_id, form_data, avatar_url)
       VALUES (?, ?, ?)
...
// ---------------------- POST อัปโหลดรูปโปรไฟล์ (สร้างแถวใหม่ถ้ายังไม่เคยกรอกแบบฟอร์ม) ----------------------
